...
class Node:
    ...
    def draw(self, surface):
        col = pg.Color('black') if self.walkable else pg.Color('red')
        px, py = self.position
        sx, sy = (self.size,) * 2
...
class Graph:
    ...
    def draw(self, surface):
        ...
        if self.target:
            ...
            pg.draw.circle(surface, pg.Color('green'), self.target, 15)
    ...
    def navigate(self):
        #jika tidak ada goal nya
        if not self.target:
            return

        # membuat path untuk agent
        for ag in self.agents:
